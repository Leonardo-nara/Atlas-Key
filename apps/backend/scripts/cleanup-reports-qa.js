...
async function main() {
  ...
  if (data.stores.length === 0 && data.users.length === 0) {
    console.log("Nenhum dado QA Reports encontrado.");
    removeManifest();
    return;
  }
  ...
  if (!applyMode) {
    ...
    console.log("Para remover: QA_REPORTS_ENV=sandbox QA_REPORTS_ALLOW_NODE_ENV_PRODUCTION=true QA_REPORTS_CLEANUP_CONFIRM=DELETE_REPORTS_QA pnpm --filter @deliveries/backend reports:qa:cleanup:prod -- --apply");
    return;
  }
  ...
  if (after.stores.length > 0 || after.users.length > 0 || after.products.length > 0 || after.sales.length > 0 || after.orders.length > 0) {
    throw new Error("Limpeza incompleta: ainda existem dados QA Reports.");
  }
  console.log("Limpeza QA Reports concluida com zero residuos principais.");
}

async function collect() {
  const manifest = readManifest();
  const storeNames = [STORE_A_NAME, STORE_B_NAME];
  const userEmails = [
    "qa-reports-store-a@example.test",
    "qa-reports-store-b@example.test",
    "qa-reports-client@example.test",
    "qa-reports-courier@example.test",
    "qa-reports-platform@example.test"
  ];
  ...
  const users = await prisma.user.findMany({
    where: { OR: [{ email: { in: userEmails } }, { id: { in: Object.values(manifest?.credentials ?? {}).map((credential) => credential.id).filter(Boolean) } }] },
